...
exports.homepage = (req, resp) => {
  resp.render("homepage", { user: req.user });
};
...
exports.updatecourse = (req, resp) => {
  userModel.course.findOne({ _id: req.params.id }).then((doc) => {
    resp.render("createcourse", { doc: doc, user: req.user });
  });
};
...
exports.delete = (req, resp) => {
  userModel.course.findOneAndDelete({ _id: req.params.id }).then(() => {
...
exports.Changepass = (req, resp) => {
  ...
  if (!valid) errors.push({ msg: "old password does not matched" });
  ...
  if (errors.length > 0) {
    ...
    userModel.signup.findOne({ _id: user._id }).then((doc) => {
      //Hash the Password
...
exports.Changeprofile = (req, resp) => {
  ...
  //check if password are same
  if (!email || !username) errors.push({ msg: "Enter all required fields" });
  if (errors.length > 0) {
    resp.render("profile", {
      errors,
      username,
